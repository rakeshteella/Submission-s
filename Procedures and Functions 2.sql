...
CREATE PROCEDURE insertConnection (
    IN connection_name VARCHAR(100)
)
BEGIN
    DECLARE new_id INT;
    
    -- Getting the count of existing records in electricity_connection_type
    SELECT COUNT(*) INTO new_id FROM electricity_connection_type;
    
    -- Adding 1 to the count to generate the new id
    SET new_id = new_id + 1;
    
    -- Inserting the new record into electricity_connection_type table
    INSERT INTO electricity_connection_type (id, connection_name) VALUES (new_id, connection_name);
    
    -- Displaying the inserted id and connection_name
    SELECT id, connection_name FROM electricity_connection_type WHERE id = new_id;
END//
